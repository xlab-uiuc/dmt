...
find_nv (const struct packed_edge * restrict IJ, const int64_t nedge)
{
  ...
  maxvtx = -1;
  for (k = 0; k < nedge; ++k) {
    if (k % 1000000 == 0) printf("find_nv %ld / %ld\n", k, nedge);
...
setup_deg_off (const struct packed_edge * restrict IJ, int64_t nedge)
{
  ...
  for (k = 0; k < 2*nv+2; ++k)
    xoff[k] = 0;
  for (k = 0; k < nedge; ++k) {
    if (k % 1000000 == 0) printf("setup_deg_off %ld / %ld\n", k, nedge);
  ...
  accum = 0;
  for (k = 0; k < nv; ++k) {
    if (k % 1000000 == 0) printf("setup_deg_off2 %ld / %ld\n", k, nv);
    int64_t tmp = XOFF(k);
  ...
  XOFF(nv) = accum;
  for (k = 0; k < nv; ++k) {
    if (k % 1000000 == 0) printf("setup_deg_off3 %ld / %ld\n", k, nv);
    XENDOFF(k) = XOFF(k);
  }
  ...
  for (k = 0; k < accum + MINVECT_SIZE; ++k) {
    if (k % 1000000 == 0) printf("setup_deg_off4 %ld / %ld\n", k, accum);
    xadjstore[k] = -1;
  }
  return 0;
}
...
fprintf(stderr, "signalling readyness to /tmp/enablement/graph500_watch\n");
  FILE *fd2 = fopen("/tmp/enablement/graph500_watch", "w+");
  ...
  for (v = 0; v < nv; ++v) {
    if (v % 1000000 == 0) printf("pack_edges %ld / %ld\n", v, nv);
    pack_vtx_edges (v);
  }
}
...
gather_edges (const struct packed_edge * restrict IJ, int64_t nedge)
{
  ...
  for (k = 0; k < nedge; ++k) {
    if (k % 1000000 == 0) printf("gather_edges %ld / %ld\n", k, nedge);
    int64_t i = get_v0_from_edge(&IJ[k]);
    int64_t j = get_v1_from_edge(&IJ[k]);
    if (i >= 0 && j >= 0 && i != j) {
      scatter_edge (i, j);
