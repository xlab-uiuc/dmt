...
#define TEA_MAX_CNT 16
#define TEA_MEMSLOT_START 3000
#define TEA_MEMSLOT_END (TEA_MEMSLOT_START + TEA_MAX_CNT)

// this should go to struct kvm but I am too lazy
static struct kvm_tea tea_list[TEA_MAX_CNT] = { 0 };

// Guest side functions

u64 kvm_hc_alloc_tea(u64 gva, u64 gpa, u64 pgcnt, u64 mode)
{
	return kvm_hypercall4(KVM_HC_ALLOC_TEA, gva, gpa, pgcnt, mode);
}
EXPORT_SYMBOL_GPL(kvm_hc_alloc_tea);

SYSCALL_DEFINE4(alloc_tea, unsigned long, gva, unsigned long, gpa, unsigned long, pgcnt, unsigned long, mode)
{
	return kvm_hc_alloc_tea(gva, gpa, pgcnt, mode);
}

// Host side functions

static void kvm_unmap_tea(struct kvm_vcpu *vcpu, int memslot, u64 gpa)
{
	// This should remove HVA as well as GPA
	srcu_read_unlock(&vcpu->kvm->srcu, vcpu->srcu_idx);
	mutex_lock(&vcpu->kvm->slots_lock);
	__x86_set_memory_region(vcpu->kvm, memslot, gpa, 0);
	mutex_unlock(&vcpu->kvm->slots_lock);
	vcpu->srcu_idx = srcu_read_lock(&vcpu->kvm->srcu);
}

static void __user *kvm_map_tea(struct kvm_vcpu *vcpu, int memslot, u64 gpa, struct page *page, u64 pgcnt)
{
	int ret = 0;
	void __user *hva;
	struct vm_area_struct *vma;
	u64 map_start, map_end, map_pgcnt;

	// Map GPA
	srcu_read_unlock(&vcpu->kvm->srcu, vcpu->srcu_idx);
	mutex_lock(&vcpu->kvm->slots_lock);
	hva = __x86_set_memory_region(vcpu->kvm, memslot, gpa, pgcnt * PAGE_SIZE);
	mutex_unlock(&vcpu->kvm->slots_lock);
	vcpu->srcu_idx = srcu_read_lock(&vcpu->kvm->srcu);

	if (IS_ERR(hva)) {
		pr_warn("TEA: map pa %llx + len %llx memslot failed %ld\n", gpa, pgcnt, PTR_ERR(hva));

		return hva;
	}

	// Test HVA
	down_read(&vcpu->kvm->mm->mmap_lock);
	vma = find_vma(vcpu->kvm->mm, (u64) hva);
	up_read(&vcpu->kvm->mm->mmap_lock);

	if (!vma) {
		pr_warn("TEA: map pa %llx + len %llx vma failed\n", gpa, pgcnt);

		kvm_unmap_tea(vcpu, memslot, gpa);
		return ERR_PTR_USR(-EINVAL);
	}

	map_start = ALIGN(vma->vm_start, PAGE_SIZE);
	map_end = ALIGN_DOWN(vma->vm_end, PAGE_SIZE);
	map_pgcnt = (map_end - map_start) / PAGE_SIZE;

	map_pgcnt = map_pgcnt < pgcnt ? map_pgcnt : pgcnt;

	if (pgcnt > map_pgcnt) {
		pr_warn("TEA: %lld pages cannot be mapped\n", pgcnt - map_pgcnt);
	}

	// Map HPA
	down_write(&vma->vm_mm->mmap_lock);
	ret = remap_pfn_range(vma, map_start, page_to_pfn(page), map_pgcnt * PAGE_SIZE, vma->vm_page_prot);
	up_write(&vma->vm_mm->mmap_lock);

	if (ret) {
		pr_warn("TEA: map pa %llx + len %llx pfn failed %d\n", gpa, pgcnt, ret);

		kvm_unmap_tea(vcpu, memslot, gpa);
		return ERR_PTR_USR(ret);
	}

	return hva;
}

static u64 kvm_free_tea(struct kvm_vcpu *vcpu, u64 gva, u64 gpa, u64 pgcnt, u64 mode)
{
	int i;
	u64 all_start, all_end, alloc_start, alloc_end;

	all_start = ktime_get_ns();

	for (i = 0; i < TEA_MAX_CNT; i++) {
		// Search the right one
		if (tea_list[i].vmid != TEA_VMID(mode) || tea_list[i].gva != gva)
			continue;

		alloc_start = ktime_get_ns();

		kvm_unmap_tea(vcpu, TEA_MEMSLOT_START + i, tea_list[i].gpa);
		free_contig_range(page_to_pfn(tea_list[i].page), tea_list[i].pgcnt);
		tea_list[i] = (struct kvm_tea) { 0 };

		alloc_end = ktime_get_ns();

		all_end = ktime_get_ns();

		pr_warn("TEA: free va %llx + len %llx succeed, %lld ns, %lld ns/MB\n", tea_list[i].gva, tea_list[i].pgcnt,
			(all_end - all_start) - (alloc_end - alloc_start), (alloc_end - alloc_start) * 256 / pgcnt);
		return 0;
	}

	pr_warn("TEA: free va %llx not found\n", gva);
	return -EINVAL;
}

static u64 kvm_alloc_tea(struct kvm_vcpu *vcpu, u64 gva, u64 gpa, u64 pgcnt, u64 mode)
{
	int i;
	u64 all_start, all_end, alloc_start, alloc_end;
	void __user *hva;
	...
	if (i >= TEA_MAX_CNT) {
		pr_warn("TEA: alloc va %llx no slot\n", gva);
		return -EAGAIN;
	}
	...
	tea_list[i].page = alloc_contig_pages(pgcnt, GFP_PGTABLE_USER, numa_node_id(), NULL);

	if (!tea_list[i].page) {
		pr_warn("TEA: alloc va %llx + len %llx no memory\n", gva, pgcnt);
		return -ENOMEM;
	}

	tea_list[i].pgcnt = pgcnt;

	// Do mapping
	hva = kvm_map_tea(vcpu, TEA_MEMSLOT_START + i, gpa, tea_list[i].page, tea_list[i].pgcnt);
	...
	pr_warn("TEA: alloc va %llx + len %llx succeed %lx, %lld ns, %lld ns/MB\n", gva, pgcnt, page_to_pfn(tea_list[i].page),
		(all_end - all_start) - (alloc_end - alloc_start), (alloc_end - alloc_start) * 256 / pgcnt);
	return 0;
}

static u64 kvm_handle_tea(struct kvm_vcpu *vcpu, u64 gva, u64 gpa, u64 pgcnt, u64 mode)
{
	// Do some basic check
	if (!vcpu || !vcpu->kvm || !vcpu->kvm->mm || !vcpu->kvm->mm->owner)
		return -EINVAL;

	// Forward nested call to host as we want host contiguity only
	if (TEA_NESTED_LVL(mode))
		return kvm_hc_alloc_tea(gva, gpa, pgcnt, mode - 1); // -1 to reduce nested level

	// Freeing TEA
	if (TEA_IS_FREE(mode))
		return kvm_free_tea(vcpu, gva, gpa, pgcnt, mode);

	return kvm_alloc_tea(vcpu, gva, gpa, pgcnt, mode);
}
...
int kvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
	...
	case KVM_HC_ALLOC_TEA: {
		ret = kvm_handle_tea(vcpu, a0, a1, a2, a3);
		break;
	}
	default:
		ret = -KVM_ENOSYS;
